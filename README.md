...
- **未提交改动（VSCode 风格）**：已暂存 / 更改 / 未跟踪三组文件列表、状态徽标（A/M/D/R/U/?）、每文件 +/− 行数、点击行展开单文件 diff、重命名 `旧 → 新`、未跟踪文件直接显示内容；☑ 分组开关可单独隐藏某组，面板与分组均可点击折叠
- **未提交改动侧边模块**：同一面板作为独立模块停靠在对话页右侧（可拖拽调整宽度、宽度与折叠状态持久化）；折叠时只显示窄把手，不渲染内容、不发起请求；宿主工具详情面板打开时自动让位
